...
function generateSourceFiles(sourceFiles, encoding = 'utf8') {
    Object.keys(sourceFiles).forEach(file => {
        ...
        try {
            ...
            source.code = source.code.replace(/\.*[Uu][Ss][Ee].+[\s]+GO[\s]+/igm, '')
        }
        catch (e) {
...
function buildNav(members) {
    let globalNav;
    let nav = '<h2><a href="index.html">{{title}}</a></h2>'.replace('{{title}}', env.conf.templates.referenceTitle || 'Home');
    const seen = {};
    const seenTutorials = {};
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    generate(env.conf.templates.referenceTitle || 'Home',
        packages.concat(
            [{
                kind: 'mainpage',
                readme: opts.readme,
